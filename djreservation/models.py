...
class Reservation(models.Model):
    ...
    RETURNED = 5
    STATUS = (
        (BUILDING, _("V príprave")),
        (REQUESTED, _("Vyžiadané")),
        (ACCEPTED , _("Schválená")),
        (DENIED, _("Odmietnutá")),
        (BORROWED, _("Vypožičané")),
        (RETURNED, _("Vratené")),
    )
    ...
    itemi = models.ForeignKey(Items, blank = True,null=True, on_delete=models.CASCADE)

    def __str__(self):
        return "%s  %s  %s  (%s to %s)" % (self.user.get_full_name(),
