...
def email_requested(reservation, user):
    send_html_email(
        _('Rezervácia prijatá: reserv-%d') % (reservation.pk),
...
def email_accepted(reservation, user):
    send_html_email(
        subject=_('Rezervácia schválená: reserv-%d') % (reservation.pk),
...
def email_denied(reservation, user):
    send_html_email(
        _('Rezervácia zamietnutá: reserv-%d') % (reservation.pk),
...
def email_borrowed(reservation, user):
    send_html_email(
        _('Rezervácia prebieha: reserv-%d') % (reservation.pk),
...
def email_returned(reservation, user):
    send_html_email(
        _('Rezervácia predmet vrátený: reserv-%d') % (reservation.pk),
